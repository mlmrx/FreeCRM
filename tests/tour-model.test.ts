...
describe('synthetic tour journeys', () => {
  ...
  it('distinguishes unmerged intelligence scenes and future controls from available CRM', () => {
    for (const id of ['knowledge', 'today', 'capabilities']) {
      expect(tourSections.find((section) => section.id === id)?.status).toBe('Development preview');
    }
  ...
  it('finds feature and delivery labels with case-insensitive, all-term search', () => {
    ...
    expect(searchTourSections('development preview').map((section) => section.id)).toEqual(['knowledge', 'today', 'capabilities']);
