...
import { getObjectStorage, objectStorageLabel } from '@/server/storage-provider';
...
export async function GET(request: Request) {
  try {
    ...
    await retryDocumentDeleteOutbox(db, getObjectStorage(), context.workspaceId);
    ...
    const storageLabel = objectStorageLabel(identity.runtimeMode);
    const runtime = identity.runtimeMode === 'device'
      ? { mode: 'device' as const, label: 'Device workspace', detail: `Local D1 + ${storageLabel} · loopback only` }
      : identity.runtimeMode === 'authjs'
        ? { mode: 'authjs' as const, label: 'Private Vercel workspace', detail: `Cloudflare D1 + ${storageLabel} · GitHub OAuth` }
        : { mode: 'cloudflare-access' as const, label: 'Private Cloudflare workspace', detail: `D1 + ${storageLabel} · Access protected` };
