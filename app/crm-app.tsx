...
import { resolveCapabilities, workspaceProfiles, type WorkspaceProfile } from '@/lib/multi-edition';
    ...
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`} aria-label="CRM navigation">
        ...
        <div className="nav-group nav-tools">
          ...
          {snapshot.capabilities.integrations.enabled && <button className={`nav-item ${view === 'integrations' ? 'active' : ''}`} onClick={() => go('integrations')}><span>⌘</span>Integrations</button>}
          {snapshot.capabilities.agentPlane.enabled && <button className={`nav-item ${view === 'agents' ? 'active' : ''}`} onClick={() => go('agents')}><span>◈</span>Agents</button>}
...
function Integrations({ snapshot, mutate, busy, refresh, notify }: { snapshot: CRMSnapshot; mutate: (type: string, payload: Record<string, unknown>, message: string) => Promise<boolean>; busy: boolean; refresh: () => Promise<void>; notify: (message: string, tone?: Toast['tone']) => void }) {
  ...
  return <><div className="integration-intro panel"><span className="integration-lock">⌘</span><div><strong>Truthful connections by design</strong><p>These two local simulators execute now. External providers stay disconnected until you supply and authorize their real credentials.</p></div><a className="secondary-button compact" href="/api/v1/export">Export backup</a></div><section className="integration-grid">{referenceConnectors.map((definition) => { const connection = snapshot.connectorConnections.find((item) => item.key === definition.key); const connected = connection?.status === 'connected'; return <article className="integration-card panel" key={definition.key}><header><span className="integration-logo csv">{definition.key === 'csv' ? 'CSV' : '↗'}</span><StatusChip status={connection?.health ?? 'disconnected'} /></header><h2>{definition.name}</h2><p>Local, credential-free reference adapter. Scopes: {definition.scopes.join(', ')}.</p><dl><div><dt>Cursor</dt><dd>{connection?.syncCursor ?? 'Not started'}</dd></div><div><dt>Auth</dt><dd>Simulated</dd></div></dl><div className="button-row">{!connected ? <button className="primary-button" disabled={simulatorBusy} onClick={() => void operateSimulator({ operation: 'connect', connectorKey: definition.key }, `${definition.name} simulator connected.`)}>Connect simulator</button> : <><button className="primary-button" disabled={simulatorBusy} onClick={() => void operateSimulator({ operation: 'sync', connectionId: connection.id }, 'Simulation completed; no external data was claimed.', true)}>Run simulation</button><button className="secondary-button" disabled={simulatorBusy} onClick={() => void operateSimulator({ operation: 'disconnect', connectionId: connection.id }, 'Simulator disconnected and credential metadata cleared.')}>Disconnect</button></>}</div></article>; })}{snapshot.integrations.map((integration) => <article className="integration-card panel" key={integration.id}><header><span className={`integration-logo ${integration.provider}`}>{integration.provider === 'google' ? 'G' : integration.provider === 'microsoft' ? 'M' : integration.provider === 'slack' ? '#' : integration.provider === 'csv' ? 'CSV' : '↗'}</span><StatusChip status={integration.status} /></header><h2>{integration.name}</h2><p>{String(integration.config.description ?? `${titleCase(integration.syncDirection)} sync adapter`)}</p><dl><div><dt>Direction</dt><dd>{titleCase(integration.syncDirection)}</dd></div><div><dt>Last sync</dt><dd>{relativeDate(integration.lastSyncAt)}</dd></div><div><dt>Auth</dt><dd>{titleCase(integration.authType)}</dd></div></dl><button className={integration.status === 'connected' ? 'secondary-button' : 'primary-button'} disabled={busy || integration.status === 'connected'} onClick={() => connect(integration)}>{integration.status === 'connected' ? 'Connected' : integration.status === 'configured' ? 'Update endpoint' : 'Configure'}</button>{integration.lastError && <small className="inline-error">{integration.lastError}</small>}</article>)}</section></>;
}
...
function Onboarding({ mutate, busy }: { mutate: (type: string, payload: Record<string, unknown>, message: string) => Promise<boolean>; busy: boolean }) {
  const choose = (profile: WorkspaceProfile, agents = false) => void mutate('workspace.update', { profile, settings: { onboardingComplete: true, agenticOnboarding: agents } }, 'Your workspace is ready. You can change this profile any time.');
  return <section className="panel onboarding-panel"><div><p className="eyebrow">SET UP YOUR SHARED PLATFORM</p><h2>How will you use FREE CRM?</h2><p>Choose a calm starting point. Profiles only change defaults—your data always stays in the same workspace.</p></div><div className="onboarding-choices"><button disabled={busy} onClick={() => choose('personal')}>Just me<small>Personal and solopreneur essentials</small></button><button disabled={busy} onClick={() => choose('business')}>My team<small>Shared sales and service workflows</small></button><button disabled={busy} onClick={() => choose('enterprise')}>My organization<small>Governance and advanced policies</small></button><button disabled={busy} onClick={() => choose('business', true)}>Humans + agents<small>Approval-first agent assistance</small></button></div></section>;
}
...
function Agents({ snapshot, refresh, notify }: { snapshot: CRMSnapshot; refresh: () => Promise<void>; notify: (message: string, tone?: Toast['tone']) => void }) {
  const [busy, setBusy] = useState(false);
  const run = async (payload: Record<string, unknown>, success: string) => { setBusy(true); try { await agentOperation(payload); await refresh(); notify(success); } catch (error) { notify(error instanceof Error ? error.message : 'Agent operation failed.', 'error'); } finally { setBusy(false); } };
...
function Agents({ snapshot, refresh, notify }: { snapshot: CRMSnapshot; refresh: () => Promise<void>; notify: (message: string, tone?: Toast['tone']) => void }) {
  ...
  const run = async (payload: Record<string, unknown>, success: string) => { setBusy(true); try { await agentOperation(payload); await refresh(); notify(success); } catch (error) { notify(error instanceof Error ? error.message : 'Agent operation failed.', 'error'); } finally { setBusy(false); } };
  const create = () => { const name = window.prompt('Agent name'); if (name) void run({ operation: 'agent.create', name, autonomy: 'approval-required', monthlyBudgetCents: 2500 }, `${name} created paused by default.`); };
  return <div className="settings-grid"><section className="panel settings-card wide"><div className="panel-head"><div><p className="eyebrow">AGENT CONTROL PLANE</p><h2>Owned, budgeted, stoppable</h2></div><button className="primary-button" disabled={busy} onClick={create}>＋ New agent</button></div>{snapshot.agents.length ? snapshot.agents.map((agent) => <div className="workflow-row" key={agent.id}><span className="workflow-icon">◈</span><span><strong>{agent.name}</strong><small>{titleCase(agent.autonomy)} · {formatMoney(agent.spentCents)} of {formatMoney(agent.monthlyBudgetCents)}</small></span><StatusChip status={agent.emergencyStoppedAt ? 'emergency_stopped' : agent.status} /><button disabled={busy || Boolean(agent.emergencyStoppedAt)} onClick={() => void run({ operation: 'agent.safety', agentId: agent.id, status: agent.status === 'active' ? 'paused' : 'active' }, agent.status === 'active' ? 'Agent paused.' : 'Agent activated.')}>{agent.status === 'active' ? 'Pause' : 'Activate'}</button><button className="danger-button" disabled={busy} onClick={() => void run({ operation: 'agent.safety', agentId: agent.id, emergencyStop: !agent.emergencyStoppedAt }, agent.emergencyStoppedAt ? 'Emergency stop cleared; agent remains paused.' : 'Emergency stop activated.')}>{agent.emergencyStoppedAt ? 'Clear stop' : 'Emergency stop'}</button></div>) : <EmptyState title="No agents yet" body="Create an approval-first agent. It starts paused with only a local read simulator." action="Create agent" onAction={create} />}</section><section className="panel settings-card"><div className="panel-head"><div><p className="eyebrow">APPROVAL QUEUE</p><h2>Human decisions</h2></div></div>{snapshot.approvals.filter((approval) => approval.status === 'pending').map((approval) => <div className="audit-row" key={approval.id}><span className="warning-dot" /><span><strong>{approval.actionSummary}</strong><small>Expires {shortDate(approval.expiresAt)}</small></span><button disabled={busy} onClick={() => void run({ operation: 'approval.decide', approvalId: approval.id, decision: 'approved' }, 'Action approved.')}>Approve</button><button disabled={busy} onClick={() => void run({ operation: 'approval.decide', approvalId: approval.id, decision: 'rejected' }, 'Action rejected.')}>Reject</button></div>)}</section><section className="panel settings-card"><div className="panel-head"><div><p className="eyebrow">RUNS & RECEIPTS</p><h2>Auditable execution</h2></div></div>{snapshot.agentRuns.slice(0, 12).map((runItem) => <div className="audit-row" key={runItem.id}><span className={runItem.status === 'succeeded' ? 'success-dot' : 'warning-dot'} /><span><strong>{titleCase(runItem.status)}</strong><small>{runItem.id.slice(0, 8)} · {shortDate(runItem.createdAt)}</small></span>{runItem.status === 'authorized' && <button disabled={busy} onClick={() => void run({ operation: 'run.execute', runId: runItem.id }, 'Local simulation executed with a receipt.')}>Execute simulator</button>}</div>)}</section></div>;
}
...
function Admin({ snapshot, mutate, refresh, busy }: { snapshot: CRMSnapshot; mutate: (type: string, payload: Record<string, unknown>, message: string) => Promise<boolean>; refresh: () => Promise<void>; busy: boolean }) {
  ...
  return <div className="settings-grid">
    <section className="panel settings-card"><div className="panel-head"><div><p className="eyebrow">WORKSPACE</p><h2>Profile & defaults</h2></div><StatusChip status={snapshot.workspace.role} /></div><form className="settings-form" onSubmit={(event) => { event.preventDefault(); void mutate('workspace.update', { name: workspaceName, timezone, currency, profile }, 'Workspace profile saved without moving your data.'); }}><label>Workspace name<input value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} maxLength={120} required /></label><label>How do you work?<select value={profile} onChange={(event) => setProfile(event.target.value as WorkspaceProfile)}>{workspaceProfiles.map((item) => <option key={item} value={item}>{item === 'personal' ? 'Just me' : item === 'business' ? 'My team' : 'My organization'}</option>)}</select></label><label className="profile-agent-option"><input type="checkbox" checked={enabled.agentPlane.enabled} readOnly /> Humans + agents is available safely in every profile</label><small>Changing profile only changes capability defaults. Records and relationships are never migrated or deleted.</small><label>Timezone<input value={timezone} onChange={(event) => setTimezone(event.target.value)} maxLength={80} required /></label><label>Currency<input value={currency} onChange={(event) => setCurrency(event.target.value.toUpperCase())} maxLength={3} required /></label><button className="primary-button" disabled={busy}>Save settings</button></form></section>
    ...
    <section className="panel settings-card wide"><div className="panel-head"><div><p className="eyebrow">CAPABILITY REGISTRY</p><h2>Modules, navigation, limits, and policy</h2></div><span>Profile: {titleCase(snapshot.workspace.profile)}</span></div>{Object.values(snapshot.capabilities).map((capability) => <div className="workflow-row" key={capability.key}><span className="workflow-icon">◇</span><span><strong>{capability.label}</strong><small>{capability.limit === null ? 'No configured record limit' : `Limit ${capability.limit.toLocaleString()}`} · {capability.navigation ? 'Navigation module' : 'Policy capability'}</small></span><label className="switch"><input type="checkbox" checked={capability.enabled} disabled={busy} onChange={(event) => void mutate('capability.update', { key: capability.key, enabled: event.target.checked }, `${capability.label} ${event.target.checked ? 'enabled' : 'disabled'} without deleting data.`)} /><i /></label></div>)}</section>
