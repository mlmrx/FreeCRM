...
import AuditViewer from '@/app/audit-viewer';
import AgentPolicyEditor from '@/app/agent-policy-editor';
import { agentPolicySafetyRevision } from '@/lib/agent-policy-client';
import {
  formatMoney,
  moduleByType,
  moduleCatalog,
...
function Agents({ snapshot, refresh, notify }: { snapshot: CRMSnapshot; refresh: () => Promise<CRMSnapshot | null>; notify: (message: string, tone?: Toast['tone']) => void }) {
  ...
  useEffect(() => {
    const now = Date.now();
    const expiries = snapshot.agents.flatMap((agent) => agent.tools.flatMap((tool) => tool.expiresAt && Date.parse(tool.expiresAt) > now ? [Date.parse(tool.expiresAt)] : []));
    if (!expiries.length) return;
    const timeout = window.setTimeout(() => setClock(Date.now()), Math.min(Math.max(0, Math.min(...expiries) - now + 1), 2_147_483_647));
    return () => window.clearTimeout(timeout);
  }, [snapshot.agents, clock]);
    ...
    <section className="panel settings-card wide">
      ...
      {snapshot.agents.length ? snapshot.agents.map((agent) => {
        ...
        return <article className="agent-control-card" key={agent.id}>
          ...
          {snapshot.capabilities.advancedPolicies.enabled && <AgentPolicyEditor agentId={agent.id} safetyRevision={agentPolicySafetyRevision(agent, clock)} onPolicySaved={async () => { if (!await refresh()) throw new Error('Workspace refresh failed.'); }} />}
        </article>;
