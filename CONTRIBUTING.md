...
4. Run `npm run check:pr -- origin/main`, then `npm run check`, before opening a pull request. If the base guard reports a stale ancestor, create a clean branch from `origin/main` and apply only the genuinely new delta; do not resolve the problem by resubmitting the aggregate branch.
