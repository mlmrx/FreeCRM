...
and the roadmap. Knowledge, adaptive Today, and capability discovery are marked
**Development preview**: they are present in development PRs, not main at the
time this tour was authored. Update the labels only after those features land.
...
Sections link to related tour scenes, never to the sealed owner workspace or
unmerged `/brain`, `/today`, and `/demo` routes. Public exit links lead to setup,
documentation, the shared roadmap, or the repository.
