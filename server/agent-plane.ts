...
export async function proposeAgentAction(db: D1Database, identity: RequestIdentity, workspace: WorkspaceContext, action: ProposedAgentAction) {
  requirePermission(workspace.workspace.role, 'agents:manage');
  bounded(action.agentId, 'agentId', 128);
  bounded(action.toolId, 'toolId', 128);
  bounded(action.requestedScope, 'requestedScope', 120);
...
export async function setAgentSafety(db: D1Database, identity: RequestIdentity, workspace: WorkspaceContext, input: { agentId: string; status?: 'active' | 'paused'; emergencyStop?: boolean }) {
  requirePermission(workspace.workspace.role, 'agents:manage');
  bounded(input.agentId, 'agentId', 128);
  if (input.status !== undefined && !['active', 'paused'].includes(input.status)) throw new ApiError(400, 'validation_error', 'status must be active or paused.');
  const now = new Date().toISOString();
...
export async function decideApproval(db: D1Database, identity: RequestIdentity, workspace: WorkspaceContext, input: { approvalId: string; decision: 'approved' | 'rejected' }) {
  ...
  bounded(input.approvalId, 'approvalId', 128);
  if (!['approved', 'rejected'].includes(input.decision)) throw new ApiError(400, 'validation_error', 'decision must be approved or rejected.');
  const now = new Date().toISOString();
...
export async function executeAuthorizedRun(db: D1Database, identity: RequestIdentity, workspace: WorkspaceContext, runId: string) {
  requirePermission(workspace.workspace.role, 'agents:manage');
  bounded(runId, 'runId', 128);
  type RunRow = { agent_id: string; tool_id: string; action_json: string; budget_reserved_cents: number; transport: string; tool_enabled: number; status: string; agent_status: string; emergency_stopped_at: string | null; monthly_budget_cents: number; spent_cents: number };
  const run = await db.prepare(`SELECT r.agent_id,r.tool_id,r.action_json,r.budget_reserved_cents,r.status,t.transport,t.enabled tool_enabled,ai.status agent_status,ai.emergency_stopped_at,ai.monthly_budget_cents,ai.spent_cents FROM agent_runs r JOIN agent_identities ai ON ai.workspace_id=r.workspace_id AND ai.id=r.agent_id JOIN agent_tools t ON t.workspace_id=r.workspace_id AND t.id=r.tool_id JOIN agent_tool_grants g ON g.workspace_id=r.workspace_id AND g.agent_id=r.agent_id AND g.tool_id=r.tool_id WHERE r.workspace_id=? AND r.id=?`).bind(workspace.workspaceId, runId).first<RunRow>();
  ...
  if (!run.tool_enabled) throw new ApiError(409, 'tool_disabled', 'The granted tool was disabled after approval.');
